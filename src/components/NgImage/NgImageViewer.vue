        ...
        <section class="ng-panel">
          ...
          <div class="ng-body">
            <aside class="analysis-column">
              ...
              <p class="scope-note">本页按缺陷类型统计，同图同类只计一次；多类缺陷分别计数。</p>
            </aside>
            <main class="inspection-column">
              <div class="section-label"><span>02 / 图像检视</span></div>
              <div class="preview-heading"><div><h3 :title="selectedImage?.name">{{ selectedImage?.name || '暂无选中图片' }}</h3></div><span class="defect-tag" v-if="selectedImage" :title="selectedImage.defect">{{ selectedImage.defect || '其他' }}</span></div>
              ...
              <NgImageComparison v-else :image="selectedImage" />
              <div class="preview-navigation"><button class="text-button" :disabled="selectedIndex <= 0" @click="moveSelection(-1)">← 上一张</button><span><b>{{ selectedIndex < 0 ? '—' : String(selectedIndex + 1).padStart(2,'0') }}</b> / {{ visibleImages.length }}</span><button class="text-button" :disabled="selectedIndex < 0 || selectedIndex >= visibleImages.length-1" @click="moveSelection(1)">下一张 →</button><button class="action-button" :disabled="!(selectedImage?.imageUrl || selectedImage?.detectedImageUrl)" @click="openImagePreview(selectedImage)">⛶ 放大检视</button></div>
              <div class="image-metadata"><div><span>{{ selectedImage?.detectedAt ? '检测时间' : '采集时间' }}</span><strong>{{ formatTime(selectedImage?.createTime) }}</strong></div><div><span>产品品牌</span><strong>{{ selectedImage?.brand || defaultBrand }}</strong></div><div><span>检测相机</span><strong>CAM {{ selectedImage?.camera ?? defaultCamera }}</strong></div></div>
              <div class="inspection-note"><i></i>原图与检测结果对照<span>缩放后可滚动查看图像细节</span></div>
            </main>
...
import NgImageComparison from './NgImageComparison.vue';
import { normalizeNgImage, resolvePointQueryValue } from './ngImageData.js';

export default {
  name: 'NgImageViewer',
  components: { NgImageComparison },
  props: {
    lineData: {
      type: Object,
      default: () => ({})
  ...
  computed: {
    distribution() {
      ...
      this.images.forEach(image => image.defectNames.forEach(name => counts.set(name, (counts.get(name) || 0) + 1)));
      const total = [...counts.values()].reduce((sum, count) => sum + count, 0);
      const colors = ['#65e6ff', '#ffcc66', '#ff9275', '#8c9bff', '#63d9b4'];
      return [...counts].sort((a,b) => b[1]-a[1]).map(([name,count],index) => ({name,count,color:colors[index%colors.length],percent:total ? count/total*100 : 0}));
    },
    ringStyle() {
    ...
    visibleImages() { return this.images.filter(image => !this.defectFilter || image.defectNames.includes(this.defectFilter)); },
  ...
  methods: {
    ...
    async loadNgImages() {
      ...
      try {
        ...
        const images = Array.isArray(res?.data) ? res.data.map(normalizeNgImage) : [];
        this.images = images;
